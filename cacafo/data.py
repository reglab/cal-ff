...
AUTH_CREDENTIALS_PATH = "./storage.json"

cache = dc.Cache(os.path.join(DATA_ROOT, ".cache"))
...
def gdrive_auth():
    ...
    if _gdrive_auth is not None:
        return _gdrive_auth
    _gdrive_auth = GoogleAuth()
    if os.path.exists(AUTH_CREDENTIALS_PATH):
        _gdrive_auth.LoadCredentialsFile(AUTH_CREDENTIALS_PATH)
        return _gdrive_auth
    _gdrive_auth.LocalWebserverAuth()
    _gdrive_auth.SaveCredentialsFile(AUTH_CREDENTIALS_PATH)
    return _gdrive_auth
...
class GoogleSheetsDataSource(DataSource):
    ...
    def download(self):
        sheets = gsheets.Sheets.from_files("./client_secrets.json", "./storage.json")
...
class DataLoopDataSource(DataSource):
    ...
    def download(self):
        os.makedirs(os.path.dirname(self.path()), exist_ok=True)
        data = dataloop.get_dataset(self.dataset_name)
